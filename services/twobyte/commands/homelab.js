...
const SUB_GROUP = new SlashCommandSubcommandGroupBuilder()
    ...
    .addSubcommand(
        new SlashCommandSubcommandBuilder()
            .setName("on")
            .setDescription("Turn the homelab on.")
    )
    .addSubcommand(
        new SlashCommandSubcommandBuilder()
            .setName("off")
            .setDescription("Turn the homelab off.")
    );
...
const SUBCOMMANDS = {
    "power": {
        "on": async (interaction) => {
            await power("turn_on")
            await interaction.reply({ content: "I've turned ON the homelab server for you!", ephemeral: true });
        },
        "off": async (interaction) => {
            await power("turn_off")
            await interaction.reply({ content: "I've turned OFF the homelab server for you!", ephemeral: true });
        }
    }
}
