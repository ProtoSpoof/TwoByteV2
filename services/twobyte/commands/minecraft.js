    ...
    new SlashCommandSubcommandGroupBuilder()
        ...
        .addSubcommand(
            new SlashCommandSubcommandBuilder()
                .setName("on")
                .setDescription("Turn the server on.")
        )
        .addSubcommand(
            new SlashCommandSubcommandBuilder()
                .setName("off")
                .setDescription("Turn the server off.")
        )
]

const SUBCOMMANDS = {
    "power": {
        "on": async (interaction, client) => {
            await power("turn_on");
            await interaction.reply({ content: "I've turned ON the Minecraft server for you!", ephemeral: true });
        },
        "off": async (interaction, client) => {
            await interaction.deferReply({ ephemeral: true });
            let serverStatus = await getServerStatus(process.env.MINECRAFT_IP);

            if (serverStatus.online === false) {
                return await interaction.followUp({
                    content: "The Minecraft server seems to be off already. If you think this is an error contact an admin.",
                    ephemeral: true
                });
            }
            ...
            if (serverStatus?.players?.online) {
                return await interaction.followUp({
                    content: "There are still people connected to the Minecraft server. If you think this is an error contact an admin.",
                    ephemeral: true
                });
            }

            await power("turn_off")
            await interaction.followUp({ content: "I've turned OFF the Minecraft server for you!", ephemeral: true });
        }
    }
}
